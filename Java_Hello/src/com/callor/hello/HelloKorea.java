 ...
 * - 클래스가 HelloKorea이면
 * 	파일이름은 HelloKorea.java로 작성을한다  
 * -클래스 이름은 첫글자가 반드시 영문대문자
 * - 영문대소문자,숫자, _ (Under bar, Under Score)이것만 사용가능함 
...
public class HelloKorea {
	...
	/*
	 * main() method 라고 부른다 
	 * 클래스를 만들고 컴파일을 수행한 후 실행을 했을때 반드시 있어야 하는 명령문
	 */
	// 명령문의 시작부분에 //를 붙이면
	// 명령문은 없는것과 같다
	public static void main(String args[]) {
		 ...
		 * 산술(연산)식 
		 * 1. 숫자 산술식
		 * 		수학의 사칙연산을 수행하고 결과를 출력한다
		 * 
		 * 2. 문자열 산술식
		 * 		" "로 묶인 문자열을 +연산하면 문자열을 연결하여 출력한다
		 * 		문자열 산술식은 +연산만 가능
		 * 
		 * 3. 문자열 + 슷자 산술식
		 * 		모든 숫자를 자동으로 문자열화 하여 
